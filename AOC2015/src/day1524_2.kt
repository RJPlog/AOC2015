	...
	for (i0 in 0..27) {
		for (i1 in 0..27) {
			for (i2 in 0..27) {
				for (i3 in 0..27) {
					for (i4 in 0..27) {
							if (i0 != i1 && i2 != i0 && i2 != i1 && i3 != i0 && i3 != i1 && i3 != i2 && i4 != i0 && i4 != i1 && i4 != i2 && i4 != i3 ) {
								weight_box1 =
									weights[i0].toInt() + weights[i1].toInt() + weights[i2].toInt() + weights[i3].toInt() + weights[i4].toInt()
								weight_product =
									weights[i0].toLong() * weights[i1].toLong() * weights[i2].toLong() * weights[i3].toLong() * weights[i4].toLong() 
								if (weight_box1 == sum_weights / 4) {
									println(" Ieration $i: Box1: $weight_box1, prod: $weight_product, items: ${weights[i0]}, ${weights[i1]}, ${weights[i2]}, ${weights[i3]}, ${weights[i4]}")
...
// Ausgabe der Lösung für Part 2
	...
	print("   Min Product: $weight_product_min @ $i0min, $i1min, $i2min, $i3min, $i4min")
}